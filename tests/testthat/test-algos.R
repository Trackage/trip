...
test_that("angle calculation works", {
   tra <- walrus818[1:12, ]
  expect_equivalent(trackAngle(tra), 
...
test_that("home distance works", {
  expect_equivalent(homedist(walrus818), c(`353` = 194119.892690059, `354` = 515915.874666597, `355` = 513435.909676174, 
  ...
  expect_equivalent(homedist(spTransform(walrus818, "+proj=longlat +datum=WGS84")),
  ...
  expect_equivalent(homedist(walrus818, home = cbind(0, 0)), 
